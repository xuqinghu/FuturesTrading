...
public class KLineGridAxisDrawing implements IDrawing {
    ...
    public void onInit(RectF contentRect, AbstractRender render) {
        ...
        if(lastDecimalPlaces!=decimalPlaces){
            ...
            initDecimalFormat(lastDecimalPlaces);
        }
        if (xLabelPaint == null) {
    ...
    private void initDecimalFormat(int decimalPlaces){
    ...
    public void onComputeOver(Canvas canvas, int minIndex, int maxIndex, float minY, float maxY) {
        ...
        final int count = render.getZoomTimes() < 0 ? Math.abs(10 * render.getZoomTimes()) + 2 : 10;
        ...
        for (int i = minIndex; i < maxIndex; i++) {
            ...
            if (i % count == 0) {
                ...
                canvas.drawText(
                        ViewUtils.TimeStamp2Date(entrySet.getEntryList().get(i).getXLabel().substring(0,10),entrySet.getTimeType()),
                        pointCache[0],
